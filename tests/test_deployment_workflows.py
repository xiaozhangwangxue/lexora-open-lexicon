...
class DeploymentWorkflowTest(unittest.TestCase):
    ...
    def test_repreparation_reuses_only_a_verified_sealed_candidate(self) -> None:
        ...
        self.assertIn("refresh-owner", workflow)
        self.assertIn("--shard-index 0 --shard-count 2", workflow)
        self.assertIn("--shard-index 1 --shard-count 2", workflow)
        self.assertIn("candidate-owner0.sqlite", workflow)
        self.assertIn("candidate-final.sqlite", workflow)
        self.assertIn("SOURCE_CANDIDATE_SHA", workflow)
        self.assertIn('assert report["terms"] == {"words": 16_000, "phrases": 4_000}', workflow)
        self.assertIn('seal --candidate "$candidate"', workflow)
        self.assertIn('stage_one "$shard" &', workflow)
        self.assertIn('seal_one "$shard" &', workflow)
        self.assertIn('wait "$pid" || status=1', workflow)
        self.assertNotIn("systemctl start", workflow)
        self.assertNotIn("systemctl enable", workflow)
        self.assertNotIn("sqlite_snapshot_manifest.py\" backup", workflow)
