...
启动分片 `1`。该模板固定 `--shard-count 2 --workers 8 --delay 16`，请求会由
Cloudflare 中转层合并为每批最多 8 个词条。核心采集每个缓存未命中词条只使用
一次 Datamuse exact 请求；两台机器的静态上限约为每天 8.64 万次，仍低于
项目设置的每天 9 万次硬上限。不要额外启动同一数据集的并行写入进程：
它们不会突破 Cloudflare 的全局免费额度限制，反而会增加 SQLite 写冲突和
上游 `429`/`504` 重试。
...
`lexora-enrich@.service` 的 `--delay 34`，不能把两台模式的 16 秒间隔原样复制。
